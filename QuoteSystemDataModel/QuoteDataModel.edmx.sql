...
-- --------------------------------------------------
-- Date Created: 03/06/2023 11:55:33
-- Generated from EDMX file: D:\Major\CGLQuoteSystem\QuoteSystemDataModel\QuoteDataModel.edmx
-- --------------------------------------------------
...
-- Creating foreign key on [Quote_Id] in table 'PolicyTerms'
ALTER TABLE [dbo].[PolicyTerms]
ADD CONSTRAINT [FK_PolicyTermQuote]
...
-- Creating non-clustered index for FOREIGN KEY 'FK_PolicyTermQuote'
CREATE INDEX [IX_FK_PolicyTermQuote]
ON [dbo].[PolicyTerms]
    ([Quote_Id]);
GO
...
-- Creating foreign key on [ProspectId1] in table 'Businesses'
ALTER TABLE [dbo].[Businesses]
ADD CONSTRAINT [FK_ProspectBusiness]
    FOREIGN KEY ([ProspectId1])
    REFERENCES [dbo].[Prospects]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO
